...
function createMission(nazwaMisji, typMisji) {
    ...
    const MojaMisja = {
        nazwa: nazwaMisji,
        typ: typMisji, // Inne typy: "Transportowa", "Kolonizacyjna"
...
const zaplanujMisje = createMission("Ekspedycja na Marsa", "Badawcza");

function dodajCzlonkaZalogi(Mojamisja) { 
    const czlonek = prompt("Podaj imię nowego członka załogi:");
    Mojamisja.zaloga.push(czlonek);
}

function zaladujSprzet(Mojamisja) {
    const sprzet = prompt("Dodaj sprzęt do załadowania:");
    Mojamisja.ladownia.narzedzia.push(sprzet);
}

function przemierzDystans(Mojamisja) {
    const odleglosc = parseFloat(prompt("Podaj odległość do przebycia w jednostkach astronomicznych (AU):"));
    Mojamisja.dystans += odleglosc;
}

dodajCzlonkaZalogi(zaplanujMisje);
zaladujSprzet(zaplanujMisje);
przemierzDystans(zaplanujMisje);
createMission();

function raportMisji(Mojamisja) {
    console.log(`*** RAPORT MISJI: ${Mojamisja.nazwa} ***`);
    console.log(`Typ misji: ${Mojamisja.typ}`); 
    console.log(`Przebyty dystans: ${Mojamisja.dystans} AU`);
    console.log(`Pozostałe zapasy: ${Mojamisja.ladownia.zapasy}%`);
    console.log("--- ZAŁOGA ---");
    console.log(`- ${Mojamisja.zaloga}`);
    console.log("--- SPRZĘT W ŁADOWNI ---");
    console.log(`- ${Mojamisja.ladownia.narzedzia}`);
}

raportMisji(zaplanujMisje);